...
void BitcoinExchange::input_init()
{
    ...
    while (std::getline(input, tmp_line))
    {
        try
        {
            if (tmp_line.find('|') == std::string::npos || tmp_line.find_first_of('|') != tmp_line.find_last_of('|') ||
...
int BitcoinExchange::check_date(std::string date)
{
    ...
    while (std::getline(ss, str, '-'))
    {
        if (idx == 0)
        {
            ...
            if (check_dateOfyear(year))
                return 1;
        }
        else if (idx == 1)
        {
            std::istringstream(str) >> month;
            if (check_dateOfmonth(month))
                return 1;
        }
        else if (idx == 2)
        {
            std::istringstream(str) >> day;
            if (check_dateOfday(year, month, day))
                return 1;
        }
        idx++;
    }
